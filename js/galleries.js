brah.galleries = brah.galleries || {
    ...
    populate: function () {
        var $galleries = $('.galleries').html('');
        $.each(brah.galleries.galleries, function (index1, gallery) {
            $galleries.append($('<div></div>')
                .addClass('header')
                .append($('<h2></h2>')
                    .text(gallery.title)));
            $galleries.append($('<p></p>')
    ...
    load: function () {
        ...
        if (pswp) {
            brah.galleries.gallery = new PhotoSwipe(pswp, PhotoSwipeUI_Default, brah.galleries.galleries[index].images, options);
            brah.galleries.gallery.init();
        }
    },

    events: function () {
        $('.image').on('click', brah.galleries.load);
    },

    init: function () {
        brah.galleries.populate();
        brah.galleries.events();
    }
};
